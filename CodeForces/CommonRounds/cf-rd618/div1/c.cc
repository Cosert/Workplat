...
int n, m; 
struct nd { int i; ld v; } li[maxn|1]; 
...
inline void solve(/*rla*/) { 
    ...
    rep(i, n) { 
        ld a; wi(a); 
        li[i]=(nd){1, a}; 
        if(i>1 && li[i].v<=li[i-1].v) { 
            li[i-1].v=(li[i-1].v*li[i-1].i+li[i].v)/(li[i-1].i+1); 
            li[i-1].i++; 
            n--, i--; 
            for(int j=i; j>1; j--) { 
                if(li[j].v<=li[j-1].v) { 
                    li[j-1].v=(li[j-1].v*li[j-1].i+li[j].v*li[j].i)/(li[j-1].i+li[j].i); 
                    li[j-1].i+=li[j].i; 
                    n--, i--; 
                } 
                else break; 
    ...
    rep(i, n) rep(j, li[i].i) printf("%.9Lf\n", li[i].v); 
} 
