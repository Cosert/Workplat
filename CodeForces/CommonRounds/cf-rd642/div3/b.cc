...
inline void init() { 
    ri(n), ri(m); 
    for(int i=1; i<=n; i++) ri(la[i]); 
    for(int i=1; i<=n; i++) ri(lb[i]); 
} 



inline void solve() { 
    sort(la+1, la+1+n), sort(lb+1, lb+1+n); 
    for(int i=1; i<=m; i++) { 
        if(la[i]<lb[n+1-i]) swap(la[i], lb[n+1-i]); 
        else break; 
    } 
    int ans=0; for(int i=1; i<=n; i++) ans+=la[i]; 
    wln(ans); 
} 
