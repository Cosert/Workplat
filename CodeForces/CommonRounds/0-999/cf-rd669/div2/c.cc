...
inline void solve() { 
    ...
    if(n-2*(n>>1)==1) q.push(n); 

    for(int i=1, a, b; i<=2*(n>>1)-1; i+=2) { 
        pstr("?", 2), wo(i, 2), wo(i+1); 
        fflush(stdout); 
        ri(a); 
        pstr("?", 2), wo(i+1, 2), wo(i); 
        fflush(stdout); 
        ri(b); 
        if(a>b) ans[i]=a, q.push(i+1), li[a].first=0; 
        else ans[i+1]=b, q.push(i), li[b].first=0; 
    } 
    ...
    while(q.size()>1) { 
        ...
        fflush(stdout); 
        ri(b); if(b==-1) return; 
        if(a>b) ans[i]=a, q.push(j), li[a].first=0; 
        else ans[j]=b, q.push(i), li[b].first=0; 
    } 

    sort(li+1, li+n+1); 
    //wo(li[n].second); 
    ans[q.front()]=li[n].second; 
