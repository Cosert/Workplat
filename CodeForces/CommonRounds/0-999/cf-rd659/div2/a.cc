...
inline void solve() { 
    string s; rep(i, m) s.push_back('a'); 
    pstr(s, 1); 

    rep(i, n) { 
        string a; 
        rep(j, li[i]) a.push_back(s[j-1]); 
        if(li[i]<m) a.push_back(s[li[i]]=='a' ? 'b':'a'); 
        rng(j, li[i]+2, m) a.push_back(s[j-1]); 
        pstr(a, 1); 
        s=a; 
    }   
} 
