...
int n; 
struct nd{ int t, to; } li[maxn|1]; 

inline void solve(/*rla*/) { 
    wi(n); 
    int ans=0; 
    rep(i, n) wi(li[i].t), wi(li[i].to); 
    int fr=0, to=li[1].to, t1=li[1].t, t2=t1+abs(li[1].to); 
    for(int i=2; i<=n; i++) { 
        if(li[i].t>=t2) { 
            ans++; 
            t1=li[i].t; 
            t2=t1+abs(to-li[i].to); 
            fr=to; 
            to=li[i].to; 
        } 
        else { 
            if(li[i].to==to) ans++; 
            else if(fr<=to && fr<=li[i].to && li[i].to<=to) ans+=(fr+abs(li[i].t-t1)<li[i].to); 
            else if(fr>to && fr>=li[i].to && li[i].to>=to) ans+=(fr-abs(li[i].t-t1)>li[i].to); 
        } 
    } 
