...
inline void solve(/*rla*/) { 
    ...
    if(m>n) sswap(a, b), swap(n, m); 
    
    /*for(int i=0; i<n; i++) if(a[i]!=b[i%m] || (i==n-1)) { 
        wln(-1); 
        return; 
    } */
    int k=lcm(n, m); 
    for(int i=0; i<k; i++) c.push_back(b[i%m]); 
    for(int i=0; i<k; i++) if(a[i%n]!=c[i]) { 
        wln(-1); 
        return; 
    } 
    psln(c); 
