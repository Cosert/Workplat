...
inline void solve(/*rla*/) { 
    string &s=rla; n=(int)s.length(), m=-1; 
    if(n==0) { wln(0); return; } 
    rep(i, n) { 
        if(s[i-1]!=m) q.push_back((nd){s[i-1], 1}), m=s[i-1]; 
        else (*q.rbegin()).n++; 
    } 
    m=(int)q.size(); 
    if(~m&1) { wln(0); return; } 
    for(int i=1, j=m; i!=j; i++, j--) if(q[i-1].k!=q[j-1].k || q[i-1].n+q[j-1].n<3) { wln(0); return; } 
    wln(q[m>>1].n+1<3? 0 : q[m>>1].n+1); 
} 
