...
inline void solve() { 
    pii ans[3]; int cnt=0; 
    if(g[1][2]==g[2][1]) { 
        if(g[n][n-1]==g[1][2]) ans[++cnt]={n, n-1}; 
        if(g[n-1][n]==g[1][2]) ans[++cnt]={n-1, n}; 
        wo(cnt); 
        rep(i, cnt) wo(ans[i].first, 2), wo(ans[i].second); 
    } 
    else if(g[n][n-1]==g[n-1][n]) { 
        if(g[1][2]==g[n][n-1]) ans[++cnt]={1, 2}; 
        if(g[2][1]==g[n][n-1]) ans[++cnt]={2, 1}; 
        wo(cnt); 
        rep(i, cnt) wo(ans[i].first, 2), wo(ans[i].second); 
    } 
    else { 
        ans[++cnt]={n, n-1}, g[n][n-1]=g[n-1][n]; 
        if(g[1][2]==g[n][n-1]) ans[++cnt]={1, 2}; 
        else if(g[2][1]==g[n][n-1]) ans[++cnt]={2, 1}; 
        wo(cnt); 
        rep(i, cnt) wo(ans[i].first, 2), wo(ans[i].second); 
    } 
} 
