...
bool check(int a) { 
    for(ll i=1; i<=pc && pri[i]*pri[i]<=a; i++) if(n%pri[i]==0) return 1; 
    return 0; 
} 

inline void solve() { 
    if(n==1) psln("FastestFinger"); 
    else if(n==2 || (n&1)) psln("Ashishgup"); 
    else if(n>4 && n%4==0 && (n/4)%2>0) psln("Ashishgup"); 
    else { 
        if((n&1) || check(n)) { 
            psln("FastestFinger"); 
            return; 
        } 
        for(int i=1; i<=pc && pri[i]*pri[i]<=n; i++) if(!(n%pri[i]) && ((n/pri[i])>1 && check(n/pri[i]))) { 
            psln("Ashishgup"); 
            return; 
        } 
        psln("FastestFinger"); 
